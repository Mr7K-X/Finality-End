...
class PlayState extends MusicBeatState
{
  ...
  function startSong():Void
  {
    ...
    if (autoUpdateRPC) DiscordClient.instance.changePresence(
      {
        details: detailsText,
        state: SONG.song + " (" + storyDifficultyText + ")",
        largeImageKey: Paths.formatToSongPath(SONG.song),
        largeImageText: SONG.song,
        smallImageKey: iconP2.getCharacter(),
        smallImageText: dad.curCharacter
      });
    #end
  ...
  function generateNotesStrumTime(data:Array<SwagSection>):Void
  {
    for (section in data)
    {
      for (songNotes in section.sectionNotes)
      {
        ...
        if (!Std.isOfType(songNotes[3],
          String)) swagNote.noteType = ChartingState.noteTypeList[songNotes[3]]; // Backward compatibility + compatibility with Week 7 charts
  ...
  override function openSubState(SubState:FlxSubState)
  {
    ...
    if (paused)
    {
      ...
      FlxTimer.globalManager.forEach((t:FlxTimer) -> if (!t.finished) t.active = false);
      FlxTween.globalManager.forEach((t:FlxTween) -> if (!t.finished) t.active = false);
    }
  ...
  override function closeSubState()
  {
    ...
    stagesFunc((s:BaseStage) -> s.closeSubState());
    if (paused)
    {
      ...
      FlxTimer.globalManager.forEach((t:FlxTimer) -> if (!t.finished) t.active = true);
      FlxTween.globalManager.forEach((t:FlxTween) -> if (!t.finished) t.active = true);
  ...
  {
    #if DISCORD_ALLOWED
    if (health > 0 && !paused && autoUpdateRPC) DiscordClient.instance.changePresence(
      {
  ...
  function resetRPC(?showTime:Bool = false)
  {
    ...
    if (showTime) DiscordClient.instance.changePresence(
      {
        details: detailsText,
        state: SONG.song + " (" + storyDifficultyText + ")",
        largeImageKey: Paths.formatToSongPath(SONG.song),
  ...
  function set_health(value:Float):Float // You can alter how icon animations work here
  {
    ...
    iconP1.animation.play((healthBar.percent < 20) ? 'lose' : (healthBar.percent > 80 ? "win" : "idle")); // If health is under 20%, change player icon to frame 1 (losing icon), otherwise, frame 0 (normal)
    iconP1.updateHitbox();
    iconP2.animation.play((healthBar.percent > 80) ? 'lose' : (healthBar.percent < 20 ? "win" : "idle")); // If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
    iconP2.updateHitbox();
    return health;
  }
  ...
  function openChartEditor()
  {
    ...
    #if DISCORD_ALLOWED
    DiscordClient.instance.changePresence({details: "Chart Editor"});
  ...
  public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float)
  {
    ...
    stagesFunc((s:BaseStage) -> s.eventCalled(eventName, value1, value2, flValue1, flValue2, strumTime));
  ...
  public function finishSong(?ignoreNoteOffset:Bool = false):Void
  {
    ...
    {
      finishTimer = new FlxTimer().start(ClientPrefs.data.noteOffset / 1000, (_) -> {
        endCallback();
      });
    }
  }
  ...
  public function playerDance():Void
  {
    ...
    if (boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 #if FLX_PITCH / FlxG.sound.music.pitch #end) * boyfriend.singDuration
      && anim.startsWith('sing') && !anim.endsWith('miss')) boyfriend.dance();
  }
