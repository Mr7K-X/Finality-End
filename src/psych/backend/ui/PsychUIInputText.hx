...
class PsychUIInputText extends FlxSpriteGroup
{
  ...
  function onKeyDown(e:KeyboardEvent)
  {
    ...
    switch (flxKey)
    {
      ...
      case DELETE: // Delete letter to the right of caret
        ...
        var lastText = text;
        if (caretIndex < 1) text = text.substr(1);
        else
      ...
      case A, O: // these support all accents
        ...
        if (_nextAccent != NONE) charCode += grave - capital + _nextAccent;
      ...
      case N: // it only supports tilde
        if (_nextAccent == TILDE) charCode += 0xD1 - 0x4E;
        else
          _typeLetter(getAccentCharCode(_nextAccent));
      ...
      default:
        if (charCode < 1) if ((charCode = getAccentCharCode(_nextAccent)) < 1) return;
  ...
  public function updateCaret()
  {
    ...
    if (selection != null && selection.exists)
    {
      ...
      if (selection.x + selection.scale.x > textObj.x + textObj.width) selection.scale.x += (textObj.x + textObj.width - selection.x - selection.scale.x);
