...
import psych.scripts.LuaUtils.LuaTweenOptions;
#if SScript
import psych.scripts.HScript;
#end
import psych.scripts.DebugLuaText;
import psych.scripts.ModchartSprite;
...
class FunkinLua
{
  ...
  public function new(scriptName:String)
  {
    ...
    Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
      ...
      if (front) LuaUtils.getTargetInstance().add(mySprite);
      else
      {
  ...
  public function initLuaShader(name:String, ?glslVersion:Int = 120)
  {
    ...
    var foldersToCheck:Array<String> = [Paths.mods('shaders/'), Paths.getSharedPath('shaders/')];
    if (Mods.currentModDirectory != null && Mods.currentModDirectory.length > 0) foldersToCheck.insert(0, Paths.mods(Mods.currentModDirectory + '/shaders/'));
