...
class ScreenshotPlugin extends FlxBasic
{
  ...
  static function saveFile(bitmap:Bitmap):Void
  {
    ...
    else
    {
      ...
      finality.util.FileUtil.writeBytesToPath(targetPath, pngData);
    }
  }
  ...
  {
    finality.util.FileUtil.createFolderIfNotExist(Constants.SCREENSHOT_FOLDER);
  }
  ...
  {
    var flashBitmap:Bitmap = new Bitmap(new BitmapData(Constants.SETUP_GAME.width, Constants.SETUP_GAME.height, false));
