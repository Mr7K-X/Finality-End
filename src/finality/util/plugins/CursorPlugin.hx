  ...
  private static function checkTexture(mode:CursorMode):Void
  {
    for (i in 0...CURSORS_DATA.length)
    {
      if (CURSORS_DATA[i].mode == mode)
      {
        ...
        FlxG.mouse.load(BitmapData.fromImage(Image.fromFile(Paths.getPath('images/${CURSORS_DATA[i].texture}.png', IMAGE, null, true))),
          CURSORS_DATA[i].scale, CURSORS_DATA[i].addX, CURSORS_DATA[i].addY);
      }
    }
  }
}
